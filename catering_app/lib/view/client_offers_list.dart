...
class _ClientOffersListViewState extends State<ClientOffersListView> {
  ...
  Future<void> _addToCart(Meal meal) async {
    try {
      cartData.addMeal(meal);

      print("Cart Content:");
      print(cartData.meals
          .map((item) => "Meal: ${item.description}, Price: ${item.price} zł")
          .toList());
      ...
      setState(() {
        _message = "Wystąpił błąd podczas dodawania produktu do koszyka!";
        _isSuccess = false;
      });
    }
  }
  ...
  void _navigateToCart() {
    final clientCartUI = ClientCartUI(context);
    final clientCartLogic = ClientCartLogic(clientCartUI, cartData);

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => CartView(logic: clientCartLogic),
      ),
    );
  }
